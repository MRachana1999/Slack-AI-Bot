...
class SlackAIAgent {
    ...
    async analyzeWithAI(memberInfo, researchData) {
        ...
        console.log('========================================\n\n');
        console.log('this.gemini exists?', !!this.gemini);
        ...
        try {
            console.log('🔍 DEBUG: analyzeWithAI called');
            console.log('🔍 DEBUG: this.gemini =', this.gemini ? 'defined' : 'UNDEFINED');
            
            const researchSummary = researchData.length > 0
                ? researchData.map(r => `${r.title}: ${r.content}`).join('\n')
                : 'Limited research data available';
            
            console.log('🔍 DEBUG: researchSummary =', researchSummary.substring(0, 200));
            ...
            console.log('🔍 DEBUG: Gemini raw response =', result.content);
            ...
            // If still wrapped in text, extract JSON between first { and last }
            const jsonMatch = cleanedResponse.match(/\{[\s\S]*\}/);
            if (jsonMatch) {
                cleanedResponse = jsonMatch[0];
            }
            
            console.log('🔍 DEBUG: cleaned JSON =', cleanedResponse.substring(0, 300));
